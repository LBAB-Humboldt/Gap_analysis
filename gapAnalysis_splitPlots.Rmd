...
For save the results in .PDF an .PNG please uncomment the lines starting with #pdf(, #png(, and #dev.off( in the 'gapAnalysis.Rmd' file
...
points(x=coordinates(DATOS2)[, 1], y=coordinates(DATOS2)[, 2], col = rgb(0.1, 0.2, 1, 0.01), cex = .1, pch = 20)

plot(colombia, main = "UBICACION DE LOS REGISTROS COLOMBIA")
plot(DATOS2, add = TRUE, pch = 20, col = rgb(0.1, 0.2, 1, 0.01), cex = 0.5)
...
densRegistros2 <- reclassify(x = densRegistros, matrix(c(q95, maxVal, q95, 0, 0 , NA), nrow = 2, ncol = 3, byrow = TRUE),
                             filename = 'density95th.tif', overwrite = TRUE)
...
plot(colombia, add = TRUE, border = 'darkgrey')
plot(richBootHQ, main = paste('BootstrapHQ:', nrow(estimateSHQ), 'cells'))
plot(colombia, add = TRUE, border = 'darkgrey')
plot(richJackHQ, main = paste('JackknifeHQ:', nrow(estimateSHQ), 'cells'))
plot(colombia, add = TRUE, border = 'darkgrey')

plot(compRichBoot, main = paste('Bootstrap:', nrow(estimateS), 'cells'))
plot(colombia, add = TRUE)
plot(compRichJack, main = paste('Jackknife:', nrow(estimateS), 'cells'))
plot(colombia, add = TRUE)
...
COMP_BOOT <- normalize01(compRichBoot)
COMP_JACK <- normalize01(compRichJack)
COMP_JACK[is.na(COMP_JACK[])] <- 0

writeRaster(DENSIDAD,"INDICE_DENSIDAD_EST.tif", overwrite=TRUE)
writeRaster(AMBIENTAL,"INDICE_AMBIENTAL_EST.tif", overwrite=TRUE)
writeRaster(COMP_BOOT,"INDICE_COMPLEMENTARIEDAD_BOOT_EST.tif", overwrite=TRUE)
writeRaster(COMP_JACK,"INDICE_COMPLEMENTARIEDAD_JACK_EST.tif", overwrite=TRUE)
...
## Completness
par(mfrow = c(1, 2))
compRichBoot[compRichBoot[] == 0] <- NA
...
#  GSI / GAP INDEX
GSI_BOOT <- (3 - DENSIDAD - AMBIENTAL - COMP_BOOT)/3
GSI_JACK <- (3 - DENSIDAD - AMBIENTAL - COMP_JACK)/3
...
par(mfrow = c(1, 1))
plot(densJack, main = 'GSI values', col = 'blue', ylim = c(0, 10), lty = 1)
lines(densBoot, col = 'red', lty = 2)
legend('topleft', legend = c('Jackknife', 'Bootstrap'), 
       lty = c(1, 2), lwd = c(1, 1), col = c('blue', 2))
